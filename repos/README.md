...
pompui.de** (currently: `punctum`). Each app gets its own GitHub repository
and is cloned into this tree; the pompui.de repo only provides the
infrastructure (compose service, nginx routing, carousel entry) around it.

> The app code is **not** part of this repo — it lives in its own repository.
> Add `repos/<app>/` build outputs to `.gitignore` if an app keeps them.

## Layout

```
repos/
  punctum/        # clone of github.com/GitMinIT/punctum (timer/stopwatch/alarm/pomodoro)
```
